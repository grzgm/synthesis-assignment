...
namespace UnitTests
{
	internal class FakeItemCategoryDB : IItemCategoryRepository
	{
		...
		{
			return new List<ItemCategoryDTO> { new ItemCategoryDTO() { Id = 1, Name = "category", ParentId = null } };
		}

		List<ItemCategoryDTO> IItemCategoryRepository.ReadAllItemSubCategories()
		{
			return new List<ItemCategoryDTO> { new ItemCategoryDTO() { Id = 2, Name = "subCategory1", ParentId = 1 },
											   new ItemCategoryDTO() { Id = 3, Name = "subCategory2", ParentId = 1 }};
		}

		ItemCategoryDTO IItemCategoryRepository.ReadItemCategory(string name, string password)
		{
			throw new NotImplementedException();
		}
